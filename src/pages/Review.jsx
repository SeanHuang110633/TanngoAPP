...
import { db, updateRecord } from '../firebase';
...
const Review = () => {
  ...
  useEffect(() => {
    console.log(`${level}Review useEffect` + currentUser);
    ...
    fetchWords();
  }, [category, currentUser, level]); // 當類別或使用者改變時重新載入
  ...
  // 更新下次複習日期 
  const doUpdateRecord = async (updatedWords) => {
    if (!currentUser) return;
    let nextReviewDate;
    try {
      ...
      // 更新 N5_record
      await updateRecord(currentUser, category, finalDate, level)
  ...
  const handleAnswer = async (wasRemembered) => {
    ...
    if (currentIndex < wordsToReview.length-1) {
      ...
      try {
        ...
        await doUpdateRecord(currentUpdatedWords);
