...
const LevelPage = () => {
  ...
  const initializeLearning = async () => {
    ...
    try {
      ...
      const categoryDocRef = doc(db, COLLECTIONS[level], `${selectedCategory}`);
      const categoryDoc = await getDoc(categoryDocRef);
      ...
      // 3. 創建學習記錄文檔
      // 文檔 ID 格式: {level}_{category}，例如 N5_noun
      const docId = `${level}_${selectedCategory}`;
      ...
      // 4. 導向複習頁面
      navigate(`/${level}/review/${selectedCategory}`);
  ...
  const handleStartLearning = async (category) => {
    ...
    if (!status) {
      ...
      if (reviewDate <= today) {
        navigate(`/${level}/review/${category}`);
      }else{
  ...
  return (
    <Layout>
      <div className={styles.levelPage}>
        <h1 className={styles.pageTitle}>{level} 單字分類</h1>
