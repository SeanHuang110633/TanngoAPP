...
import { getN4Record, db, COLLECTIONS } from '../firebase';
...
const N4Page = () => {
  ...
  const fetchCategoryTotalWords = async (category) => {
    try {
      const categoryDocRef = doc(db, COLLECTIONS.N4, `${category}`);
  ...
  const initializeLearning = async () => {
    ...
    try {
      ...
      const categoryDocRef = doc(db, COLLECTIONS.N4, `${selectedCategory}`);
      const categoryDoc = await getDoc(categoryDocRef);
      ...
      // 3. 創建學習記錄文檔
      // 文檔 ID 格式: N4_{category}，例如 N4_noun
      const docId = `N4_${selectedCategory}`;
      ...
      // 4. 導向複習頁面
      navigate(`/n4/review/${selectedCategory}`);
  ...
  return (
    <Layout>
      <div className={styles.levelPage}>
        <h1 className={styles.pageTitle}>N4 單字分類</h1>
